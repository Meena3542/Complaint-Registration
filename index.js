...
app.get("/stats/total-users", async (req, res) => {
  try {
    const count = await UserSchema.countDocuments({ userType: "Ordinary" });
    res.json({ count });
  } catch (error) {
...
app.get("/stats/total-agents", async (req, res) => {
  try {
    const count = await UserSchema.countDocuments({ userType: "Agent" });
    res.json({ count });
  } catch (error) {
...
app.get("/stats/complaints", async (req, res) => {
  try {
    const pending = await ComplaintSchema.countDocuments({ status: "pending" });
    const resolved = await ComplaintSchema.countDocuments({ status: "completed" });
...
const transporter = nodemailer.createTransport({
  ...
  auth: {
    ...
    pass: "your-app-password",          // 🔁 create App Password (not your Gmail password)
  },
});
